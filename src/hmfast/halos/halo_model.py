...
class HaloModel:
    ...
    def cl_1h_masked(self, tracer1, tracer2, l, m, z, mask_mz, k_damp=0.01):
        ...
        def slice_z(i):
            ...
            u_sq = u_sq * damp
            # Mass integral as Simpson over ln M (matches tszpower's
            # _integrate_mz which uses simpson on logm_grid).
            mass_integrand = u_sq * (dndlnm[:, i] * mask_mz[:, i])[None, :]  # (Nl, Nm)
            return _simpson_nonuniform(mass_integrand, logm, axis=-1)        # (Nl,)
        ...
        # z-integral via Simpson (matches tszpower's simpson on z_grid).
        return _simpson_nonuniform(integrand, z, axis=0)
