...
class GNFWPressureProfile(PressureProfile):
    ...
    def __init__(self, x=None, P0=8.130, c500=1.156, alpha=1.0620, beta=5.4807, gamma=0.3292, B=1.4):
        ...
        if x is not None:
            ...
            # Match tszpower/profiles.py:14 (_X_GRID): the Hankel transform of
            # the GNFW pressure profile is sensitive to the large-x tail at low
            # ell. Truncating at x_max ~ 4 produces an ell-dependent ~5% deficit
            # at low ell. Using x in [1e-6, 1e6] with 1024 log-spaced points
            # matches the tszpower convention.
