...
register_pytree_node(MassDefinition, MassDefinition.tree_flatten, MassDefinition.tree_unflatten)
    ...
    #     """
    #     omega_m = halo_model.emulator.omega_m(z, params=params)
    #     x = omega_m - 1.0
    ...
    # #@partial(jax.jit, static_argnums=(0,1))
    # def delta_conversion_function(self, halo_model, z, m_new, m_old, delta_old, delta_new, c_old, reference=None, params=None):
    #     """
    ...
    #     r_old = jnp.squeeze(self._r_delta_scalar(halo_model, m_old, z, delta=delta_old, reference=self.reference, params=params))
    #     r_new = jnp.squeeze(self._r_delta_scalar(halo_model, m_new, z, delta=delta_new, reference=self.reference, params=params))
    ...
    # # #@partial(jax.jit, static_argnums=(0,1))
    # def convert_m_delta(self, halo_model, m, z, delta_old, delta_new, c_old, reference=None, x0=None, max_iter=20, params=None):
    #     """
    ...
    #     m, z, c_old, x0 = jnp.broadcast_arrays(m, z, c_old, x0)
    
    #     # Solve for a single set (scalar z, m, c_old, x0)
    #     def solve_single(z_i, m_i, c_i, x0_i):
    #         F = lambda m_new: self.delta_conversion_function(halo_model, z_i, m_new, m_i, delta_old, delta_new, c_i, reference=reference, params=params)
    ...
    # #@partial(jax.jit, static_argnums=(0,1))
    # def r_delta(self, halo_model, m, z, delta=None, reference=None, params=None):
    #     """
    ...
    #     if reference == "mean":
    #         rho_ref *= halo_model.emulator.omega_m(z, params=params)
            
    #     return (3.0 * m / (4.0 * jnp.pi * delta * rho_ref))**(1./3.)


    # def _r_delta_scalar(self, halo_model, m, z, delta, reference, params=None):
    #     """
    #     Internal math engine: Expects numeric delta and reference.
    #     Designed to handle scalars or broadcastable arrays without forced 2D reshaping.
    #     """
    #     # Get base critical density
    #     rho_ref = halo_model.emulator.critical_density(z, params=params)
        
    #     # Apply reference (critical vs mean)
    #     if reference == "mean":
    #         rho_ref *= halo_model.emulator.omega_m(z, params=params)
            
    #     # Apply numeric overdensity
    #     return (3.0 * m / (4.0 * jnp.pi * delta * rho_ref))**(1./3.)
